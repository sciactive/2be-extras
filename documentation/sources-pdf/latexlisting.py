# -*- coding: utf-8 -*-
"""
    pygments.formatters.latexlisting
    ~~~~~~~~~~~~~~~~~~~~~~~~~

    Formatter for LaTeX lstlisting output.
...
__all__ = ['LatexListingFormatter']


# Some tricky placement of replacers, because they'll replace each other.
def escape_tex(text):
    return text.replace('$', '$$'). \
                replace('\\', '$\\backslash$'). \
                replace('$$', '\\$'). \
...
class LatexListingFormatter(Formatter):
    r"""
    Format tokens as LaTeX code. This needs the `listings` and `color`
    standard packages.
    ...
    The `get_style_defs()` method of a `LatexListingFormatter` returns a
    string containing ``\def`` commands defining the macros needed inside
    ...
    `verboptions`
        Additional options given to the Verbatim environment (see the *listings*
    ...
    """
    name = 'LaTeX Listing'
    aliases = ['latexlisting', 'texlisting']
    filenames = ['*.tex']
